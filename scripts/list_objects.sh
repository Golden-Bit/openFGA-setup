...
payload_path="${ROOT_DIR}/examples/listobjects.json"

resp="$(curl -sS -X POST "${FGA_API_URL}/stores/${store_id}/list-objects" \
  -H "Authorization: Bearer ${FGA_API_TOKEN}" \
  -H "content-type: application/json" \
  -d @"${payload_path}")"

echo "${resp}" | tee "${RUNTIME_DIR}/listobjects.json" >/dev/null
echo "OK"
